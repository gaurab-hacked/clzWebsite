...
const PreRegistation = () => {
  ...
  const preRegistrationFormSubmit = (e) => {
    ...
    // for father name
      ...
      setValError({ stream: "", name: "", DOB: "", fName: "Name must be greater than 5-20 character", mName: "", pAddress: "", lAddress: "", phNumber: "", email: "" })
      validation = false
    }
    // for mother name
    else if (registerVal.mName.length <= 5 || registerVal.mName.length>=20) {
      setValError({ stream: "", name: "", DOB: "", fName: "", mName: "Name must be greater than 5-20 character", pAddress: "", lAddress: "", phNumber: "", email: "" })
      validation = false
    }
    else if (registerVal.pAddress.length <= 5 || registerVal.pAddress.length>=25) {
      setValError({ stream: "", name: "", DOB: "", fName: "", mName: "", pAddress: "Address must be greater than 5-25 character", lAddress: "", phNumber: "", email: "" })
      validation = false
    }
    else if (registerVal.lAddress.length <= 5 || registerVal.lAddress.length>=25) {
      setValError({ stream: "", name: "", DOB: "", fName: "", mName: "", pAddress: "", lAddress: "Address must be greater than 5-25 character", phNumber: "", email: "" })
      validation = false
    }
